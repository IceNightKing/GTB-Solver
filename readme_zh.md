...
- \[Add\] 新增了玩家预警控制模式
- \[Add\] 新增了玩家预警控制增强模式
- \[Fix\] 修复了用户设置 `MULTI_LANG = "zh"` 后造成的 `NameError` 崩溃
- \[Opt\] 适配了 Hypixel 服务器《建筑大师》游戏模式新增加的聊天称号
- \[Opt\] 现在程序会将匹配式对应字数与日志辅助处理模式抓取到的当前回合主题字数进行对比，并在二者不相等时发出通知
- \[Opt\] 优化了程序输出语言的部分翻译
- \[Opt\] 词库更新
...
🔹 支持使用正则表达式进行猜测  
🔹 适配多语言词库，支持使用多语言进行猜测  
🔹 支持对比游戏内玩家已经猜测的主题，避免重复猜测  
🔹 默认按照词频统计指数降序输出，提高猜测正确率  
🔹 支持规避胜场排名靠前玩家  
...
> [!TIP]
> 在默认匹配模式下，输入空格、连字符或大于 7 的数字后仅会匹配到英语，输入任意简体中文后仅会匹配到简体中文
...
5.00 - Greek Temple - 希腊神庙
5.00 - Solar Panels - 太阳能电池板
4.50 - Build Battle - 建筑大师
...
> [!NOTE]
> 启用随机复制模式需要一并开启自动复制模式。自动复制模式的开启方法详见 `配置修改方法 4`
...
- 输出样式示例（其中，`3` 为当前回合数，`10` 为总回合数，`NoticeYou` 为当前回合建筑师名称，`13` 为包含空格的当前回合主题字数）

  ``` TXT
  [3/10] NoticeYou(13)
  ```

- 对比猜测示例（以 Conveyor Belt 为例）
  ...
  请输入匹配式：8 4
  该主题英语字数为 13 个字母
  13.00 - Swimming Pool - 泳池 - Basen
  7.50 - Wrecking Ball - 破坏球 - Sloopkogel
  4.50 - Conveyor Belt - 传送带 - Forderband
  0.00 - Presents Pile - 礼物堆
  ```

  ``` TXT
  [3/10] NoticeYou(13)
  NoticeMW: basen
  检测到有玩家猜测了主题 泳池 但未猜对，即将据此输出筛选后的匹配条目
  7.50 - Wrecking Ball - 破坏球 - Sloopkogel
  4.50 - Conveyor Belt - 传送带 - Forderband
  0.00 - Presents Pile - 礼物堆
  ```
  ...
  检测到有玩家猜测了主题 泳池、破坏球 但未猜对，即将据此输出筛选后的匹配条目
  4.50 - Conveyor Belt - 传送带 - Forderband
  0.00 - Presents Pile - 礼物堆
  ```

> [!TIP]
> 日志辅助处理模式支持处理服务器语言设置为简体中文、繁体中文、日语、韩语、俄语、德语、法语、西班牙语、葡萄牙语、意大利语、英语的情况，并支持对比玩家使用简体中文、繁体中文、日语、韩语、俄语、德语、法语、西班牙语、葡萄牙语、意大利语、英语、Shortcut(s)、Multiword(s) 猜测的主题。若您的词库文件版本不同，实际对比效果可能会发生改变
...
> [!TIP]
> 建议将该间隔设置在 `0.01` ~ `0.20` 秒的范围内。若您发现开启日志辅助处理模式后游戏相关状态更新不及时或未能成功抓取玩家猜测的主题，可适当减小重复读取间隔
...
> [!NOTE]
> 自动复制模式与日志辅助处理模式开启时，自定义内容才会被复制。自动复制模式的开启方法详见 `配置修改方法 4`，日志辅助处理模式的开启方法详见 `配置修改方法 6`
...
> [!NOTE]
> 启用主题辅助记录模式需要一并开启日志辅助处理模式。日志辅助处理模式的开启方法详见 `配置修改方法 6`
...
> [!NOTE]
> 启用半自动发送模式需要一并开启自动复制模式与日志辅助处理模式。自动复制模式的开启方法详见 `配置修改方法 4`，日志辅助处理模式的开启方法详见 `配置修改方法 6`
...
> [!TIP]
> 该间隔是在减去重复猜测剩余冷却时间后额外的暂停时间，建议将该间隔设置在 `1.0` ~ `5.0` 秒的范围内。该间隔设置过小可能会触发服务器反垃圾信息机制而被踢出游戏
...
- 默认关闭。如需实时预警等待房间的后续进入玩家，并在玩家预警控制列表内的玩家进入等待房间后发出通知，请在 `GTB-Solver_main.py` 内找到以下代码，替换 `False` 为 `True` 即可
...
> [!NOTE]
> 启用玩家预警控制模式需要一并开启日志辅助处理模式。日志辅助处理模式的开启方法详见 `配置修改方法 6`

### 16. 修改玩家预警控制增强模式状态

- 默认关闭。如需在玩家预警控制列表内的玩家进入等待房间后自动返回游戏大厅，请在 `GTB-Solver_main.py` 内找到以下代码，替换 `False` 为 `True` 即可
...
> [!NOTE]
> 启用玩家预警控制增强模式需要一并开启玩家预警控制模式与半自动发送模式。玩家预警控制模式的开启方法详见 `配置修改方法 15`，半自动发送模式的开启方法详见 `配置修改方法 12`

### 17. 修改玩家预警控制离线列表文件路径

- 默认路径为 `GTB-Solver_main.py` 同文件夹。如需修改玩家预警控制离线列表文件路径，请在 `GTB-Solver_main.py` 内找到以下代码，替换引号内的路径即可（支持中文路径）
...
- 默认加入《建筑猜猜乐》总胜场排名前 `500` 位的玩家至玩家预警控制列表内。如需修改玩家预警控制模式阈值，请在 `GTB-Solver_main.py` 内找到以下代码，修改数值即可
...
> [!TIP]
> 该阈值目前可设置在 `1` ~ `1000` 位的范围内

### 19. 修改玩家预警控制模式黑名单

- 默认为空。如需额外加入玩家至玩家预警控制列表内，请在 `GTB-Solver_main.py` 内找到以下代码，在列表内添加玩家名即可
...
- 默认为空。如需删除玩家预警控制列表内可能出现的玩家，请在 `GTB-Solver_main.py` 内找到以下代码，在列表内添加玩家名即可
